from fastapi import APIRouter, HTTPException, Depends
import logging
from ..auth import get_token_then_APIS_cached
from ..weaviate_pool import get_weaviate_client

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

router = APIRouter()    

def delete_visiting_student_schema(weaviate_url=None, weaviate_api_key=None, openai_api_key=None, user_id=None):
    """Delete the visiting student schema if it exists."""
    try:
        client = get_weaviate_client(weaviate_url, weaviate_api_key, openai_api_key, user_id)
        if "Visiting_students" in client.collections.list_all():
            client.collections.delete("Visiting_students")
            logger.info("Deleted existing VisitingStudent schema")
            return True
        return False
    except Exception as e:
        logger.error(f"Error deleting VisitingStudent schema: {e}")
        raise

@router.delete("/visiting-students-deletion/schema")
async def delete_schema(api_data=Depends(get_token_then_APIS_cached)):
    """Endpoint to delete the visiting student schema. Use with caution as this will delete all visiting student data."""
    try:
        user_id = api_data["user_id"]
        weaviate_url = api_data["weaviate_url"]
        weaviate_api_key = api_data["weaviate_api_key"]
        openai_api_key = api_data["openai_api_key"]
        
        deleted = delete_visiting_student_schema(weaviate_url, weaviate_api_key, openai_api_key, user_id)
        if deleted:
            return {"status": "success", "message": "VisitingStudent schema deleted successfully"}
        return {"status": "not_found", "message": "VisitingStudent schema does not exist"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) 